...
class ProfilePageTbCell: UITableViewCell {
    ...
    @IBOutlet weak var contactAndDirectionView: UIView!
    ...
    @IBOutlet weak var storeTimingView: UIView!
    ...
    func setData(otherUserDetailsData:UserDetailsModel) {
        ...
        storeTimingView.isHidden = timings.isEmpty
...
//        }
        ...
        viewDirections.isHidden = otherUserDetailsData.address_visible == 1 ? false : true
        
        if (otherUserDetailsData.show_email_on_app ?? false == false) ||  (otherUserDetailsData.phone?.isEmpty == false && otherUserDetailsData.phone != nil){
            self.viewContact.isHidden = false
        }else{
            self.viewContact.isHidden = true
        }
        
        let isEmailHidden = otherUserDetailsData.show_email_on_app ?? false
        let isPhoneMissing = otherUserDetailsData.phone?.isEmpty ?? true
        let isAddressHidden = (otherUserDetailsData.address_visible ?? 0) == 0

        if isEmailHidden && isPhoneMissing && isAddressHidden {
            contactAndDirectionView.isHidden = true
        } else {
            contactAndDirectionView.isHidden = false
        }
