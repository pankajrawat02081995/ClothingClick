...
class NewCategoryDetailsVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        debugPrint("This is cat id : -> \(self.catID)")
        self.setupTableView()
    }
    ...
    @IBAction func clearAllOnPress(_ sender: UIButton) {
        ...
        let array = FilterSingleton.share.filter.selectedCategories?.components(separatedBy: ",").filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }
        let filterArray = array?.filter { !(ids.contains($0)) == true }
        FilterSingleton.share.filter.selectedCategories = filterArray?.filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }.joined(separator: ",").trimmingCharacters(in: .whitespacesAndNewlines)
...
extension NewCategoryDetailsVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if FilterSingleton.share.filter.selectedCategories?.components(separatedBy: ",").contains("\(indexData?.category_id ?? 0)") == true{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var selectedIDs = FilterSingleton.share.filter.selectedCategories?
        ...
        if indexPath.row == 0 {
            ...
            FilterSingleton.share.filter.selectedCategories = selectedIDs.joined(separator: ",")
        } else {
    ...
    private func updateFilterCategorySelections() {
        let currentSelectedIDs = FilterSingleton.share.filter.selectedCategories?
            .components(separatedBy: ",")
            .filter { !$0.isEmpty } ?? []
...
//            dict.removeValue(forKey: "selectedCategories")
            dict["categories"] = self.catID ?? ""
