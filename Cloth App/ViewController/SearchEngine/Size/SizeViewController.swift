...
class SizeViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.isFilterProduct == true || isSaveSearch == true{
...
extension SizeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        else
        {
            ...
            if (self.saveSearch == false || self.isFilterProduct == false) && isSaveSearch == false {
                self.callViewCount()
            }
        }
    }
                ...
                if error == nil {
                    if let response = response {
                        if let data = response.dictData{
                            ...
                            if self.isFilterProduct == true || self.isSaveSearch == true{
                                self.btnViewItems.setTitle("Add", for: .normal)
                            }else{
                                    if self.saveSearch {
                                        self.btnViewItems.setTitle("Add to saved Search", for: .normal)
