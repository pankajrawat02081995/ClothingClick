...
class DiscoverVC: BaseViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        var filter = FilterSingleton.share.filter.toDictionary()

        filter?.removeValue(forKey: "brand_id")
        filter?.removeValue(forKey: "cat_id")

        FilterSingleton.share.filter = filter?.toModel(Filters.self) ?? Filters()
...
extension DiscoverVC{
    ...
    func callHomeList() {
        ...
        let gender: String
        switch FilterSingleton.share.genderSelection {
        case 0:
            gender = "1"
        case 1:
            gender = "2"
        ...
        let sizes = appDelegate.isMySelectedSize
