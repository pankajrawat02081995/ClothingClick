...
//        }
        ...
        param?["sizes"] = appDelegate.isMySelectedSize
        param?.removeValue(forKey: "slectedCategories")
        param?.removeValue(forKey: "categories")
        
        if listType == "3" || listType == "5" || listType == "6"{
            param?.removeValue(forKey: "sizes")
            if  listType == "3"{
                param?.removeValue(forKey: "gender_id")
            }
        }
...
//            param.removeValue(forKey: "slectedCategories")
    ...
    func callPopularBrandPost(isShowHud: Bool, page: String) {
        if appDelegate.reachable.connection != .none {
            ...
            if let filterParams = FilterSingleton.share.filter.toDictionary() {
                
                // 🔹 Step 3: Merge filter into param
                for (key, value) in filterParams {
                    
                    // ✅ Only overwrite if filter has valid value
            ...
            // 🔹 Step 4: Ensure critical params are not overridden incorrectly
            param["latitude"] = appDelegate.userLocation?.latitude ?? ""
            param["longitude"] = appDelegate.userLocation?.longitude ?? ""
            
//            let searchID = [
//                "save_search_id": "\(self.saveSearchId ?? 0)",
//                "page": "\(page)"
//            ]
            
            APIManager().apiCallWithMultipart(
                of: HomeListDetailsModel.self,
                isShowHud: isShowHud,
                URL: BASE_URL,
                apiName: APINAME.BRAND_POST.rawValue,
                parameters: param
            ) { (response, error) in
                ...
                if error == nil {
                    ...
                    self.lblNoData.isHidden = !self.postList.isEmpty
                    self.CVProducts.reloadData()
                    
                    if self.sort {
                        self.sort = false
                        if !self.postList.isEmpty {
                            self.CVProducts.scrollToItem(
                    ...
                    UIAlertController().alertViewWithTitleAndMessage(
                        self,
                        message: error?.domain ?? ErrorMessage
                    )
                }
            }
