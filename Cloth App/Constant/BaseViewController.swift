                ...
                if error == nil {
                    if let response = response {
                        if let userDetails = response.dictData {
                            ...
                            debugPrint(sizes)
                            appDelegate.selectGenderId = "\(userDetails.user_selected_gender ?? 0)" == "1" ? "0" : "\(userDetails.user_selected_gender ?? 0)" == "2" ? "1" : ""
                            FilterSingleton.share.genderSelection = userDetails.user_selected_gender ?? 0 == 1 ? 0 : (userDetails.user_selected_gender ?? 0) == 2 ? 1 : nil
                            FilterSingleton.share.filter.gender_id = "\(userDetails.user_selected_gender ?? 0)" == "1" ? "0" : "\(userDetails.user_selected_gender ?? 0)" == "2" ? "1" : ""
                            debugPrint("\(userDetails.user_selected_gender ?? 0)")
                            FilterSingleton.share.filter.sizes = sizes
                            appDelegate.isMySelectedSize = sizes
                ...
                if error == nil {
                    if let response = response {
                        if response.status == kIsSuccess {
                            if let userDetails = response.dictData {
//                              appDelegate.isMySelectedSize = userDetails.
                                self.saveUserDetails(userDetails: userDetails)
                                self.startWithAuth(userData: userDetails)
                            }
                        }
